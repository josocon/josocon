...
if (!Session::isLoggedIn ()) {
	...
	$picture = $db->getItemPictureById ($id);
	if (!$picture) {
		throw new \Exception ('picture not found');
	}
	
	$db->removeItemPicture ($picture);
	$item = $db->getItemById ($picture->item_id);
	if (!$item) {
...
print_header ('/delete-item-picture/', $item->name);
?>
<josocon-markdown><?= escape ($picture->description) ?></josocon-markdown>
<section class='form-wrapper edit-form-wrapper'>
<h2>写真の削除</h2>
<form class='edit-form input-form' action='/delete-item-picture/' method='POST'>
