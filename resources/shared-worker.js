...
const factor = async input => {
	...
	const timer = msg => {
		const time = +new Date;
		console.info ('%d: %c%s', timings.length, 'font-weight: bold', msg);
		if (timings.length > 0) {
			console.log (
	...
	if (n.lesser (1000000)) {
		...
		for (let i = 2; i < 1000; i++) {
			...
			if (0 == (n % i)) {
				return [i.toString (), (n / i).toString ()];
			}
		}
		return [n.toString ()];
	}
	...
	for (let offset = 0; offset < max_offset; offset += block_size) {
		...
		for (let i = 0; i < block_size; i++) {
			vector[i] = ceil_sqrt_n.add (i).add (offset).pow (2).subtract (n);
				...
				for (let j of js) {
					if (j < block_size) {
						...
						if (tmp_vector[j].equals (1)) {
							smoothValues[smoothCounter] = vector[j];
	...
	for (let basis of kernel) {
		...
		if (comparison && x.notEquals (y)) {
			...
			console.log ('factorization found:', p, q);
			timer ('Finished!');
			await Promise.resolve ();
			return [p, q];
		} else {
